...
class DecisionEngine:
    ...
    def _collect_signals(self, context, emotion, patterns, memory, base):
        ...
        passive_crisis_score = patterns.get("passive_crisis_score", 0)

        hopelessness_score = patterns.get("hopelessness_score", 0)

        isolation_score = patterns.get("isolation_score", 0)

        negative_score = patterns.get("negative_language_score", 0)

        return {
            "crisis_signal": (
                crisis.get("crisis_probability", 0) >= 0.5
                or patterns.get("crisis_language_score", 0) > 0
                or passive_crisis_score >= 0.6
            ),
            "critical_crisis": (
                crisis.get("is_emergency", False)
                or (passive_crisis_score >= 0.6 and hopelessness_score >= 0.5)
                or (
                    hopelessness_score >= 0.7
                    and patterns.get("conversation_escalation", {}).get(
                        "escalation_detected", False
                    )
                )
            ),
            "passive_crisis_detected": passive_crisis_score > 0,
            "isolation_detected": isolation_score > 0,
            "hopelessness_detected": hopelessness_score > 0,
            "negative_language_detected": negative_score > 0,
            "context_crisis_override": context.get("crisis_override", False),
            ...
            "context_risk_high": context.get("contextual_risk_score", 0) >= 0.55,
        }
    ...
    def _calculate_score(self, base, context, signals):
        ...
        score = base_score * 0.50 + context_score * 0.50
        ...
        if signals["crisis_signal"]:
            ...
            reasoning.append("Crisis signal increased safety priority")
        ...
        if signals["context_crisis_override"]:
            ...
            reasoning.append("Context crisis override increased safety priority")
        ...
        if signals["passive_crisis_detected"] and signals["hopelessness_detected"]:
        ...
        return (round(min(score, 1), 4), reasoning)
    ...
    def _actions(self, level):
        ...
        actions = {
            ...
            "Mild Concern": ["Supportive monitoring recommended"],
    ...
    def decide(
        ...
        return {
            ...
            "requires_human_review": (score >= 0.55),
